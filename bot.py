...
async def botcreation(ctx):
	...
	if xmas<now:
		...
		embed.add_field(name=":question::question::question: Bot Creation :question::question::question:",
			value=f"{now.year+1} ago, {dev} started making me! @everyone")
		await ctx.send(embed=embed)
	delta=xmas-now
	weeks, remainder=divmod(int(delta.total_seconds()), 604800)
	days, remainder2=divmod(remainder, 86400)
...
async def prestiemilia(ctx):
	...
	if xmas<now:
		...
		embed.add_field(name=":question::question::question: Couple Love :question::question::question:",
			value=f"{now.year+1} ago, {presti} & {emilia} became a couple! @everyone")
		await ctx.send(embed=embed)
	delta=xmas-now
	weeks, remainder=divmod(int(delta.total_seconds()), 604800)
	days, remainder2=divmod(remainder, 86400)
...
async def game_default(self):
	...
	games = [f"Use {bot_prefix}help for help!", f"{sum(1 for _ in self.bot.get_all_members())} users | {len(self.bot.guilds)} servers", f"Give us feedback? Use: {bot_prefix}feedback [message]"]
	current_number = 0
	while True:
